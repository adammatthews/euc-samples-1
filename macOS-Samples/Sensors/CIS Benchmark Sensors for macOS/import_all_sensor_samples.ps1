...
# Version 1.4
  ...
  .SYNOPSIS
    ...
    This has been amended to upload Mac OS Samples only. 
    MUST RUN AS ADMIN

  .DESCRIPTION
    Place this PowerShell script in the same directory of all of your samples (.sh files) or use the -SensorsDirectory parameter to specify your directory. 
...
Function Set-Sensors($SensorName, $Description, $QueryType, $Script, $ResponseType, $Context) {
...
# TODO: Parse .sh Files (macOS Sensors)
Function Get-GetMacOSSensors {
    Write-Host("Parsing PowerShell Scripts")
    $PSSensors = Select-String -Path $SensorsDirectory\*.sh -Pattern 'Return Type' -Context 10000000
...
$headerv2 = @{ #Update-Sensor and Set-Sensor APIs require V2. 
    "Authorization"  = "Basic $cred";
    "aw-tenant-code" = $WorkspaceONEAPIKey;
